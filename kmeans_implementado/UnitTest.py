...
class MyTestCase(unittest.TestCase):
    ...
    global list_points
    list_points = [Point(np.array([1, 1])), Point(np.array([1, 3])), Point(np.array([3, 1])), Point(np.array([3, 3]))]
    global cluster
    ...
    def testGetNearestCluster(self):
        self.assertEquals(KMeans.get_nearest_cluster([cluster, Cluster([Point(np.array([8, 8]))])], point), 0)
